...
### 11.8 Registry Schema (The SSOT Handbook)
...
#### 11.8.1 โครงสร้างระดับ Top-level
| Key | คำอธิบาย |
|---|---|
...
#### 11.8.2 โครงสร้างภายใต้ id_naming_conventions
...
#### 11.8.3 การทำงานร่วมกับ Scaffolding Script
...
## 12. Operational Commands (Template)

### 12.1 MSP / GKS commands (บังคับมี)
...
### 12.2 App (ขึ้นกับ stack — ตัวอย่าง generic)
...
### 12.3 Pre-commit (via husky หรือ equivalent)
...
### 12.4 Slash Commands (แนะนำสำหรับ Claude Code / Gemini)
...
## 13. Path Encoding & Multi-Agent Coordination
...
## 14. Minimum Viable Bootstrap (การเริ่มจากศูนย์)
...
touch CLAUDE.md GEMINI.md registry.yaml system_config.yaml CHANGELOG.md
```
...
- `scripts/msp/build-l0-index.mjs` — สแกนทุก root folders ของ gks/ เพื่อหา atomic files → JSONL
- `scripts/msp/validate-atomic-ids.mjs` — dedup + frontmatter
- `scripts/msp/runner-microtask.mjs` — T1 runner (ถ้าจะใช้ SLM codegen)
...
- เขียน `CLAUDE.md` (T3 instruction) และ `GEMINI.md` (T2 instruction)
...
- (optional) `gks/microtasks/FEAT-001/*.task.yaml`
- code → `src/...`
- entry ใน `CHANGELOG.md`

---

## 15. Checklist สำหรับ Fork ไปใช้งาน

```
[ ] แทนที่ YourProject / YRP / ExampleFeature ทั้งหมดด้วยชื่อจริง
[ ] อัปเดต path encoding (D--yourproject → path จริง)
[ ] ลบ section 11 (ID) ที่ไม่ใช้ หรือเพิ่ม domain-specific ID ของคุณ
[ ] ตั้งค่า stack ที่ §4 / §9 (framework, lang, ORM)
[ ] เขียน .brain/msp/LLM_Contract/atomic_contract.yaml
[ ] เขียน .brain/msp/LLM_Contract/codegen_microtask_contract.yaml
[ ] สร้าง _templates/ ใน microtasks/ ตาม framework
[ ] เลือก agent ประจำ tier (T3/T2/T1) + ลงทะเบียน
[ ] setup Obsidian REST API + MCP
...
## 16. Principles — หลักคิด 7 ข้อที่คงไว้เสมอ
