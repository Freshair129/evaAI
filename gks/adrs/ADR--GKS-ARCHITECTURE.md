...
# ADR--GKS-ARCHITECTURE — Treat GKS as 4-Layer Knowledge System (Truth + Indexes + Protocol + Agents)
...
> **คำชี้แจงสำคัญ**: ADR นี้**ไม่ได้แทน GKS** — GKS (folder, ID convention, frontmatter, registry) ยังเหมือนเดิมทุกอย่าง สิ่งที่ ADR นี้กำหนดคือ **architecture ของ GKS** — วิธีจัดวาง layer ระหว่าง truth (markdown) กับ access pattern (providers + protocol + agents)

---

## Decision

**ออกแบบ GKS เป็นระบบ 4 layer แยก concern ชัด** (truth + indexes + protocol + agents)

### นิยาม 4-layer GKS Architecture
...
### D. 4-layer GKS architecture (chosen)
...
ถ้า 4-layer architecture complex เกินไป:
